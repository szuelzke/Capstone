...
def filter_transactions(account_id):
    if 'user_id' in session  and session.get('mfa_completed', False):
        ...
        if account:
            # get transactions 
            transactions_list = db_session.query(Transaction).filter(account_id=account_id).filter_by(account_id=account_id).filter(Transaction.date.between(start_date, end_date)).all()
            return render_template('transactions.html',transactions=transactions_list, user=user, account=account, categories=categories)
        else:
            return redirect(url_for('add_account'))
    else:
